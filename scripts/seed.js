...
// scripts/seed.ts
var import_payload2 = require("payload");
...
var Media = {
  ...
  upload: {
    staticDir: import_path.default.resolve("./public/media"),
    imageSizes: [
      {
        name: "thumbnail",
        width: 400,
...
// src/payload.config.ts
var import_meta = {};
var filename = (0, import_url.fileURLToPath)(import_meta.url);
var dirname = import_path2.default.dirname(filename);
var payload_config_default = (0, import_payload.buildConfig)({
  ...
  typescript: {
    outputFile: import_path2.default.resolve(dirname, "payload-types.ts")
  },
...
async function seed() {
  const payload = await (0, import_payload2.getPayload)({ config: payload_config_default });
  for (const work of works_default) {
    ...
    for (const filename2 of work.images) {
      const imagePath = import_path3.default.join(__dirname, "data/images", filename2);
